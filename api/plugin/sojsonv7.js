...
// 创建共享的沙箱上下文（用于 virtualGlobalEval）
...
// 临时 eval 函数（每次创建新上下文，模拟原 ivm 的临时 isolate）
function evalOneTime(str) {
  const tempSandbox = { console };
  vm.createContext(tempSandbox);
  return vm.runInContext(String(str), tempSandbox);
}
